...
use state::{ChatMessage, ClientState, ClientStatePathExt, client_state, client_theme, this_entity, this_player};
#[cfg(feature = "debug")]
use wgpu::Device;
...
impl Client {
    fn can_use_cinematic_dialog(&self, npc_id: EntityId) -> bool {
        ...
        entities.iter().any(|entity| matches!(entity, Entity::Player(_))) && entities.iter().any(|entity| entity.get_entity_id() == npc_id)
    }

    fn is_cinematic_dialog_active(&self) -> bool {
        self.client_state.follow(client_state().cinematic_dialog_window()).is_active()
    }
    ...
    fn cinematic_dialog_targets(&self) -> Option<(Point3<f32>, Point3<f32>, Direction)> {
        ...
        let entities = self.client_state.follow(client_state().entities());
        let player = entities.iter().find(|entity| matches!(entity, Entity::Player(_)))?;
    ...
    fn should_continue_cinematic_dialog(&self, npc_id: EntityId) -> bool {
        self.is_cinematic_dialog_active() && self.can_use_cinematic_dialog(npc_id)
    }
    ...
    fn close_cinematic_dialog_window(&mut self) {
        self.client_state.follow_mut(client_state().cinematic_dialog_window()).end();
        self.interface.close_window_with_class(WindowClass::CinematicDialog);
    }

    fn close_dialog_windows(&mut self) {
    ...
    fn render_frame(&mut self, event_loop: &ActiveEventLoop) {
            ...
            match event {
                ...
                NetworkEvent::LoginServerDisconnected { reason } => {
                    if reason != DisconnectReason::ClosedByClient {
                        #[cfg(feature = "debug")]
                        print_debug!("Disconnection from the login server with error");

                        // Reconnecting transparently here would wipe the previously
                        // created auth_node on the login server (rAthena explicitly
                        // resets the session when an account logs in again while
                        // `char_server == -1`). The result is that the next time the
                        // user picks a character server, the char-server forwards
                        // stale `login_id1`/`login_id2` and the login server replies
                        // with REFUSED. Instead, drop back to the login screen and
                        // let the user retry manually so a fresh auth_node is in
                        // place by the time they pick a server.
                        #[cfg(not(feature = "debug"))]
                        self.interface.close_all_windows();
                        ...
                        self.interface
                            .open_window(ErrorWindow::new("Lost connection to the login server.".to_owned()));
                    }
                }
                ...
                NetworkEvent::ResurrectPlayer { entity_id } => {
                    ...
                    // Reset the death animation of whoever was resurrected so they
                ...
                NetworkEvent::ChangeMap { map_name, position } => {
                    ...
                    self.client_state
                        .follow_mut(client_state().entities())
                        .retain(|entity| matches!(entity, Entity::Player(_)));
                    self.client_state.follow_mut(client_state().dead_entities()).clear();
                    self.client_state.follow_mut(client_state().ground_items()).clear();
                    *self.client_state.follow_mut(client_state().buffered_action()) = None;
                    ...
                    // A map change is also how the server signals respawn after death
                    // (the client gets teleported to the save point). Close the
                    // respawn window and reset the player's animation so they don't
                    // arrive at the new map still stuck in the dead pose.
                    self.interface.close_window_with_class(WindowClass::Respawn);
                ...
                NetworkEvent::ChatMessage {
                    ...
                    let local_player_origin = origin_entity_id.or_else(|| {
                        let player_name = self.client_state.follow(client_state().player_name());
                        let local_message_prefix = format!("{player_name} : ");

                        text.strip_prefix(&local_message_prefix)
                            .filter(|message| !message.starts_with('@'))
                            .and_then(|_| self.client_state.try_follow(this_entity()).map(|entity| entity.get_entity_id()))
                    });

                    if let Some(origin_entity_id) = local_player_origin
                        && let Some(entity) = self
                            .client_state
                            .follow_mut(client_state().entities())
                            .iter_mut()
                            .find(|entity| entity.get_entity_id() == origin_entity_id)
                    {
                        entity.set_overhead_message(text.clone(), client_tick);
                    }
                ...
                NetworkEvent::AddCloseButton { npc_id } => {
                    ...
                    if self.should_continue_cinematic_dialog(npc_id) {
                        self.client_state
                            .follow_mut(client_state().cinematic_dialog_window())
                            .initialize(npc_id)
                            .add_close_button();
        ...
        if cinematic_keyboard_advance_pressed {
        ...
        for event in input_events {
            match event {
                ...
                InputEvent::ResetCameraRotation => {
                    if !cinematic_dialog_active {
                        self.player_camera.reset_rotation();
                    }
                }
                ...
                InputEvent::ToggleShowInterface => {
                ...
                InputEvent::AdvanceCinematicDialog => {
                    let advance = self.client_state.follow_mut(client_state().cinematic_dialog_window()).advance();
        ...
        if cinematic_dialog_active {
            let text_sound_count = self
                .client_state
                .follow_mut(client_state().cinematic_dialog_window())
                .update_typewriter(delta_time as f32);
            ...
            if text_sound_enabled {
                for index in 0..text_sound_count.min(4) {
        ...
        if let Some(map) = self.map.as_ref() {
            ...
            let use_cinematic_camera = *self
                ...
                && cinematic_dialog_targets.is_some();
            #[cfg(feature = "debug")]
            let use_cinematic_camera = use_cinematic_camera && !render_options.use_debug_camera;

            if let Some((player_position, npc_position, player_direction)) = cinematic_dialog_targets
                && use_cinematic_camera
            {
                if !self.cinematic_camera_active {
                    #[cfg(feature = "debug")]
                    ...
                    self.cinematic_camera
                        .set_immediate_to(source_camera.camera_position(), source_camera.focus_point());
                }
            ...
            self.cinematic_camera_active = use_cinematic_camera;
            ...
            //  Sprites and Interface
            {
                ...
                let overhead_message_background_color = *self.client_state.follow(client_theme().window().background_color());
                let overhead_message_text_color = *self.client_state.follow(client_theme().text().color());

                self.client_state.follow(client_state().entities()).iter().for_each(|entity| {
                    entity.render_overhead_message(
                        &self.middle_interface_renderer,
                        current_camera,
                        screen_size,
                        client_tick,
                        overhead_message_background_color,
                        overhead_message_text_color,
                    );
                });
                ...
                let mut interface_frame = {
                    ...
                    // window is open.
                    if !cinematic_dialog_active
                        && (!interface_has_focus || !interface_frame.input_characters(&self.client_state, &input_report.characters))
