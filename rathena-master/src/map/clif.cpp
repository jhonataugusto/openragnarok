...
static int32 clif_parse_WantToConnection_sub(int32 fd)
{
	...
	// Compare against the expected packet size from the packet table.
	// Using RFIFOREST(fd) here would fail when the client batches multiple
	// packets in a single TCP segment (e.g. open-source clients like
	// Korangar that send the connect packet immediately followed by a
	// keep-alive). We only need to ensure the buffer contains AT LEAST the
