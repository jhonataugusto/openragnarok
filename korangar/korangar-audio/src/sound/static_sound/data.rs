...
impl StaticSoundData {
    ...
    /// Sets the pitch of the sound, clamped to a safe playback range.
    #[must_use]
    pub(crate) fn pitch(&self, pitch: f32) -> Self {
        let mut new = self.clone();
        new.settings.pitch = sanitize_pitch(pitch);
        new
    }
...
mod tests {
    ...
    fn static_sound_data() -> StaticSoundData {
        StaticSoundData {
            sample_rate: 44_100,
            frames: Arc::from([Frame::from_mono(1.0)]),
            settings: StaticSoundSettings::default(),
        }
    }

    #[test]
    fn pitch_returns_clone_with_safe_pitch() {
        let data = static_sound_data();

        let low = data.pitch(0.1);
        let high = data.pitch(3.0);
        let invalid = data.pitch(f32::NAN);

        assert_eq!(data.settings.pitch, 1.0);
        assert_eq!(low.settings.pitch, 0.5);
        assert_eq!(high.settings.pitch, 2.0);
        assert_eq!(invalid.settings.pitch, 1.0);
    }
}
