...
use ragnarok_packets::{Packet, PacketExt, PacketHeader};
...
fn packet_header_text(bytes: &[u8]) -> String {
    match bytes {
        [low, high, ..] => format!("0x{:04X}", u16::from_le_bytes([*low, *high])),
        _ => "<cut off>".to_owned(),
    }
}

fn packet_to_raw_bytes<P: Packet>(packet: &P) -> Vec<u8> {
    let mut byte_writer = ByteWriter::with_encoding(UTF_8);
    match packet.packet_to_bytes(&mut byte_writer) {
        Ok(_) => byte_writer.into_inner(),
        Err(error) => format!("<serialization failed: {error:?}>").into_bytes(),
    }
}
...
impl PacketHistory {
    pub fn new() -> (PacketHistory, PacketHistoryCallback) {
        ...
        let _ = std::fs::remove_file(PACKET_TRACE_FILE);
...
impl PacketCallback for PacketHistoryCallback {
    ...
    fn incoming_packet_bytes(&self, bytes: Vec<u8>) {
        append_packet_trace(Direction::Incoming, "OK", "Parsed", &bytes);
    }
    ...
    where
        Packet: ragnarok_packets::Packet,
    {
        let raw_bytes = packet_to_raw_bytes(packet);
        append_packet_trace(Direction::Outgoing, "OK", std::any::type_name::<Packet>(), &raw_bytes);
        let packet: Packet = packet.clone();
    ...
    fn unknown_packet(&self, bytes: Vec<u8>) {
        append_packet_trace(Direction::Incoming, "UNKNOWN", "Unknown", &bytes);
    ...
    fn failed_packet(&self, bytes: Vec<u8>, error: Box<ConversionError>) {
        append_packet_trace(Direction::Incoming, "ERROR", "Error", &bytes);
