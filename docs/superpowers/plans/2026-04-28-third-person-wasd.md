...
**Goal:** Build an optional third-person WASD movement mode in Korangar while preserving the current click-to-move gameplay when disabled.
...
- [ ] Add `ThirdPersonCamera` with smoothed focus, smoothed distance clamp, yaw, pitch clamp, and view/projection generation.
...
- [ ] Generate WASD movement immediately, then at most every 500ms while held.
