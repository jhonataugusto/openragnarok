...
fn overhead_message_packet_to_event(packet: OverheadMessagePacket) -> NetworkEvent {
    NetworkEvent::ChatMessage {
        text: packet.message,
        color: MessageColor::Broadcast,
        origin_entity_id: Some(packet.entity_id),
    }
}
...
mod tests {
    ...
    #[test]
    fn overhead_message_keeps_origin_entity_id() {
        let event = overhead_message_packet_to_event(OverheadMessagePacket {
            entity_id: EntityId(42),
            message: "Tester : hello".to_owned(),
        });
    ...
    Callback: PacketCallback,
{
    ...
    packet_handler.register(overhead_message_packet_to_event)?;
    ...
    packet_handler.register_noop::<PartyMemberHealthPointsPacket>()?;
