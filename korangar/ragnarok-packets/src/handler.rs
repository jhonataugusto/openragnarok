...
pub trait PacketCallback: Clone + 'static {
    ...
    /// Called by the [`PacketHandler`] with the exact bytes consumed for a
    /// successfully parsed incoming packet.
    ...
    Callback: PacketCallback,
{
    ...
    pub fn process_one(&mut self, byte_reader: &mut ByteReader) -> HandlerResult<Output> {
        ...
        match handler(byte_reader) {
            Ok(output) => {
                let packet_end = byte_reader.get_offset();
                self.packet_callback
                    .incoming_packet_bytes(byte_reader.bytes_in_range(packet_start, packet_end));

                HandlerResult::Ok(output)
            }
